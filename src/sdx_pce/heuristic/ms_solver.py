...
class MSSolver:
    ...
    def initialize_optimization_variables(self):
        print(f"tunnel constaints:{len(self.mip.constraints)}") 
        for tunnel in self.network.tunnels.values():
            tunnel.init_flow_var(self.mip)
        print(f"{len(self.network.tunnels)} tunnel constaints:{len(self.mip.constraints)}") 
...
class FFCSolver(MSSolver):
    def __init__(self, mip, network):
        MSSolver.__init__(self, mip, network)
        print(f"Before Demand constaints:{len(self.mip.constraints)}") 
        for demand in self.network.demands.values():
    ...
    def add_demand_constraints(self):
        ...
        print(f"Demand constaints:{len(self.mip.constraints)}")    
    ...
    def add_edge_capacity_constraints(self):
        ...
        print(f"Edge constaints:{len(self.mip.constraints)}")  

                                      
class DualFFCSolver(MSSolver):
